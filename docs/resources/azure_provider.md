...
-> **Note:** The service principal must have Contributor access to the subscription or resource group where you will deploy YugabyteDB nodes.
...
The details for configuration are available in the [YugabyteDB Anywhere Configure Cloud Provider Documentation](https://docs.yugabyte.com/stable/yugabyte-platform/configure-yugabyte-platform/set-up-cloud-provider/azure/).
...
If you previously managed Azure providers with the deprecated [`yba_cloud_provider`](cloud_provider) resource (`code = "azu"`), switch the block type, lift `azure_config_settings.*` to top-level fields, and re-import the state:
...
```sh
terraform state rm yba_cloud_provider.azure
terraform import yba_azure_provider.azure <provider-uuid>
```

Notable schema differences:

- `azure_config_settings.{client_id, client_secret, tenant_id, subscription_id, resource_group, network_subscription_id, network_resource_group, hosted_zone_id}` are now top-level fields.
