...
If you previously managed AWS providers with the deprecated [`yba_cloud_provider`](cloud_provider) resource, switch the block type, lift `aws_config_settings.*` to top-level fields, and re-import the state:
...
- `aws_config_settings.{access_key_id, secret_access_key, use_iam_instance_profile, hosted_zone_id}` are now top-level fields.
