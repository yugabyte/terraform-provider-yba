...
You can configure the GCP VPC in three ways:
...
The details for configuration are available in the [YugabyteDB Anywhere Configure Cloud Provider Documentation](https://docs.yugabyte.com/stable/yugabyte-platform/configure-yugabyte-platform/set-up-cloud-provider/gcp/).
...
- `ssh_private_key_content` (String, Sensitive) SSH private key content to access YugabyteDB nodes. Must be set together with ssh_keypair_name (self-managed mode). If both fields are omitted, YugabyteDB Anywhere generates and manages the key pair (YBA-managed mode). Stored in Terraform state - use an encrypted backend for security.
...
## Migration from `yba_cloud_provider`

If you previously managed GCP providers with the deprecated [`yba_cloud_provider`](cloud_provider) resource, switch the block type, lift `gcp_config_settings.*` to top-level fields, and re-import the state:
...
```sh
terraform state rm yba_cloud_provider.gcp
terraform import yba_gcp_provider.gcp <provider-uuid>
```

Notable schema differences:

- `gcp_config_settings.{credentials, project_id, network, shared_vpc_project_id, use_host_vpc, use_host_credentials, create_vpc, yb_firewall_tags}` are now top-level fields.
- Subnets are configured at the region level via `regions.shared_subnet`. GCP zones are computed only (auto-discovered by YBA) and no longer need to be declared in HCL.
- The top-level `code`, `dest_vpc_id`, `host_vpc_id`, and `host_vpc_region` fields are not used.
...
```
~ zones {
    ~ code   = "us-central1-c" -> "us-central1-b"
      subnet = ...
  }
- zones {
    - code   = "us-central1-b" -> null
    ...
  }
```
